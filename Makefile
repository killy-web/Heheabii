...
run:
	...
	@echo -e "\e[34m######## Building and Running instainfo with Docker-compose ########\e[0m"
...
build-run-testing:
	...
	@echo -e "\e[34m######## Building and Running instainfo with Docker-compose for Testing/Debugging ########\e[0m"
	@[ -d config ] || { echo -e "\e[31mConfig folder not found! Please run 'make setup' before running this command.\e[0m"; exit 1; }
	@echo -e "\e[34m[#] Killing old docker processes\e[0m"
	@docker-compose rm -fs || exit 1
	@echo -e "\e[34m[#] Building docker container\e[0m"
	...
	@docker-compose run --name instainfo-testing -d --rm --entrypoint "sleep infinity" instainfo || exit 1
	@echo -e "\e[32m[#] instainfo-test container is now Running!\e[0m"

cleanup-testing:
	@echo -e "\e[34m######## Cleanup Build-run-testing Container ########\e[0m"
	@docker-compose down
	@echo -e "\e[32m[#] instainfo-test container has been removed\e[0m"
